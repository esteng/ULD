...
def evaluate_model(model_dir, audio_dir):
	...
	for model, model_path in models:
		...
		for (fea_path, top_path, textgrid_path) in zipped_paths_list:
			...
			pred_frame_labels = model.decode(data, tops)
			...
			true_frame_labels = read_tg(textgrid_path, len(pred_frame_labels))
			...
			pred_frame_labels_all.extend(pred_frame_labels)
			true_frame_labels_all.extend(true_frame_labels)
