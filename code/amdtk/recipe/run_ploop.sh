...
delay=20                         # number of seconds to wait for the server to start
...
#### PHONE-LOOP MODEL ####
n_units=$(cat data/idx_phone_map.txt | wc -l | sed 's/ //g') || exit 1
...
if [ ! -f ${out_dir}/fea_loader/.done ]; then
    ...
    python3 utils/create_features_loader.py \
        --log_level ${log_level} \
        --profile ${profile} \
        --njobs ${njobs} \
        --delay ${delay} \
...
if [ ! -f ${out_dir}/init_model/.done ]; then
    ...
    python3 utils/create_phone_loop.py \
        --log_level ${log_level} \
        --n_units ${n_units} \
        --n_states ${n_states} \
        --n_comp ${n_comp} \
...
for subset in ${decode_sets}; do
    ...
    if [ ! -f ${out_dir}/decode/${subset}/labels/.done ]; then
        ...
        python3 utils/decode.py \
            ...
            ${out_dir}/fea_loader/fea_loader.bin \
            ${out_dir}/fea_loader/data_stats.bin \
            ${out_dir}/training/model.bin \
            data/${subset}/fea_list.txt \
