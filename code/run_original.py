...
if train: 
	...
	optimizer.run(fea_paths, callback)
	end = systime.time()
...
for (fea_path, top_path) in zipped_paths:
    ...
    result_intervals= model.decode(data, state_path=True)
    print("---")
    print("Phone sequence for file", fea_path, ":")
    print(result_intervals)

    import itertools
    groups = itertools.groupby(result_intervals)
    for_tg = []
    for g in groups:
    	phone = g[0]
    	if len(for_tg ) == 0: 
    		...
    		start = for_tg[-1][2]
    	dur = sum(1 for _ in g[1])
    	end = start+dur 
    	for_tg.append((phone, start, end))
    result_intervals = for_tg
    #print(result_intervals)
